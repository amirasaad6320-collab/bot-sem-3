...
// 1️⃣ التوكن الخاص بك
const bot = new Telegraf('8974042053:AAHwLxS9A1BQwmS_FKFWLYI6kWVD_yelJ1k');
...
await ctx.replyWithMediaGroup([
        { type: 'document', media: 'BQACAgQAAxkBAAM6aqB-nHYV0-mMCnKkywWVAAHN2Ap-AAJDHAACtqXpUj6aUy7cNEnOPQQ' },
        { type: 'document', media: 'BQACAgQAAxkBAAIBw2qiC0ExXYygDyo1Aare0KRZSdcnAAIJIwAC1qQRUWnHKFDOe4xgPQQ' }
    ]);
});
...
bot.hears('OSPE 🔬', async (ctx) => {
    ctx.reply('OSPE Section', Markup.keyboard([ '2028 OSPE','pathology ospe','⬅️ Main Menu']).resize());
});
...
bot.hears('pathology ospe', async (ctx) => {
    await ctx.replyWithMediaGroup([
        { type: 'document', media: 'BQACAgQAAxkBAANIaqCBwPWRiU0hMHPhHDyB5j93__8AAlMcAALH4chSuGQVyrEEdOI9BA' }
    ]);
});
...
bot.hears('🎥 Videos', (ctx) => {
    ctx.reply('🎥 **Videos Section**', Markup.keyboard([ 'glucometer','skin fold','anatomy jars','blood sample','histo slides','⬅️ Main Menu']).resize());
});
...
bot.hears('histo slides', async (ctx) => {
    const videoIds = [
        "CgACAgQAAxkBAAMSap9Ev91r0WiVq9WGBVcaM6r-SfQAAt4eAAL292lT7sASiuL1eHk9BA",
        "CgACAgQAAxkBAAMTap9Ev78rjAI6W2d3M3EqW6se6_UAAtseAAL292lTB2tfjOJzX0M9BA",
        "CgACAgQAAxkBAAMUap9Ev4xw-pFtNdO6tpkjPL1o4tQAAuAeAAL292lTE4j1O0TYohA9BA",
        "CgACAgQAAxkBAAMVap9EvzdaNCTCNj1xcgn7c6yuT3gAAtweAAL292lTFFONlJCZfMU9BA",
        "CgACAgQAAxkBAAMWap9Ev-gQQj-yrPUBi1cgbm7sI6gAAt0eAAL292lTk3zRREkP91Y9BA",
        "CgACAgQAAxkBAAMXap9Ev8MSkY8nLqrfN1xbbirtjGcAAt8eAAL292lT60En-5aDtBA9BA",
        "CgACAgQAAxkBAAMYap9Ev-x8t1qo4nPLXtbV1x4G4UMAAtoeAAL292lTxs2K9BRx9Rk9BA"
    ];

    for (const id of videoIds) {
        try {
            await ctx.replyWithVideo(id);
        } catch (err) {
...
bot.hears('anatomy jars slides', async (ctx) => {
    await ctx.replyWithMediaGroup([
        { type: 'document', media: 'BQACAgQAAxkBAAM0aqA-yHs54PuwuaG-563_jbCXTsMAAmYdAAK2peFSQl-HnAeSJuI9BA' },
...
bot.on('video', (ctx) => {
    ...
    console.log("VIDEO ID IS:", fileId);
    ctx.reply(`Video ID: ${fileId}`);
});

// 3️⃣ إعداد Vercel Serverless Function (مهم جداً للرفع)
module.exports = async (req, res) => {
    try {
        if (req.method === 'POST') {
            ...
            res.status(200).send('Bot is active on Vercel Webhook!');
        }
    } catch (error) {
