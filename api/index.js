...
bot.hears('Mid 1 2029', async (ctx) => {
    await ctx.replyWithDocument('BQACAgQAAxkBAAMIap9CiIyeCNJU4WmkuYNHrC-jwu4AAhseAAJByrhTLTyoIXKL1gc9BA'); // مثال
});

bot.hears('Mid 2 2029', async (ctx) => {
    await ctx.replyWithDocument('BQACAgQAAxkBAAMKap9C1sILzgNRDn3Nsgp_Svg1NzAAApodAALdSyhRB_N2hNEgp509BA'); // مثال
});
...
bot.hears('2029', async (ctx) => {
    await ctx.replyWithDocument('BQACAgQAAxkBAAM6aqB-nHYV0-mMCnKkywWVAAHN2Ap-AAJDHAACtqXpUj6aUy7cNEnOPQQ'); // مثال
});
bot.hears('2027', async (ctx) => {
    await ctx.replyWithDocument('BQACAgQAAxkBAAMqap9VbadbxKMSAmP7ejae5AhtGIMAApEhAALrr7lQav5VMeqd3CE9BA'); // مثال
});
bot.hears('2026', async (ctx) => {
    await ctx.replyWithDocument('BQACAgQAAxkBAAMsap9VdO7vg5CBD7D5CbHK7AhqK1kAApUhAALrr7lQy4uGyBmIjPk9BA'); // مثال
});
bot.hears('anatomy', async (ctx) => {
    await ctx.replyWithDocument('BQACAgQAAxkBAAMuap9VhA5oh0C1SMA0z-yQwMt3xlcAApQhAALrr7lQiKH9I5iA3TY9BA'); // مثال
});
...
bot.hears('histology', async (ctx) => {
    await ctx.replyWithDocument('BQACAgQAAxkBAANMaqCCH1obL1ltj9FEmIppoDZdxI4AAmMbAAJ8sEFTgCfOFFCxDaA9BA'); // مثال
    } );
bot.hears('pathology', async (ctx) => {
    await ctx.replyWithDocument('BQACAgQAAxkBAANNaqCCH6NAg-7fcne3Ehxk_6RzNm0AAmUbAAJ8sEFTwwJxopwoFzs9BA'); // مثال
}
);
...
bot.hears('2028 OSPE', async (ctx) => {
    await ctx.replyWithDocument('BQACAgQAAxkBAANKaqCBzfWZ2kW6U9XUNL-LUzHcw6MAAkEcAAK2pelSz6RkWdkw8x89BA'); // مثال
}
);
...
bot.hears('✍️ Quizzes', async (ctx) => {
    await ctx.replyWithDocument('BQACAgQAAxkBAAMCap9BO1iWkdz3MNAds6adedPVoz0AAuAaAAJ7oCFSb5_xkjqeTRE9BA'); // مثال
});

// 6️⃣ قسم Videos (فيديوهات مباشرة بالـ ID)
